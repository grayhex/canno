...
## 10. Структура статических ассетов

Для кастомной графики и шрифтов используйте каталоги:

- `static/images/` — изображения (включая логотип по умолчанию `static/images/logo1.png`),
- `static/icons/custom/` — пользовательские иконки,
- `static/fonts/custom/` — пользовательские шрифты.

В админке поле «Путь к логотипу» должно ссылаться на файл внутри проекта (например `static/images/my-logo.png`).


## 11. Проверка доступности после деплоя

После запуска проверьте:

- главная страница `http://<SERVER_IP>:8000/`,
- вход в панель `http://<SERVER_IP>:8000/admin/login`,
- доступность логотипа `http://<SERVER_IP>:8000/logo.png` (берётся из `homepage_logo_path`),
- прямой вход участника по коду `http://<SERVER_IP>:8000/play/<access_code>`.
